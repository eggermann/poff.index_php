...
function cmsHandleEditAction(): void
{
    ...
    if ($action === 'prompt') {
        ...
        $sharedWorkSystemPrompt = [
            ...
            'Use work.type for the base family and work.template for the exact template override. Use work.templateMap as inherited MIME => template defaults for child items, and only apply a mapped template when the item MIME matches the key or its family wildcard. When a worktype exposes fields such as autoplay, loop, muted, poster, fit, background, or caption, set those as config values on work instead of hardcoding them into the template unless the user explicitly asks for one-off markup.',
